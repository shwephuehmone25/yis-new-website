...
beforeEach(() => {
  window.history.replaceState({}, "", "/mandalay");
});
afterEach(() => {
...
test("Mandalay uses one shared header, main, and footer", () => {
  ...
  expect(
    screen.getByRole("heading", { name: "Academic programs" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("heading", { name: "Schoolwide events" }),
  ).toBeInTheDocument();
  expect(
    screen.getAllByRole("link", {
      name: "Yangon International School home",
    })[0],
...
test("contact form prepares an email draft without claiming delivery", () => {
  ...
  fireEvent.change(screen.getByLabelText("Phone"), {
    target: { value: "+959123456789" },
  });
  fireEvent.change(screen.getByLabelText("Preferred tour date"), {
    target: { value: "2026-10-01" },
  });
  fireEvent.change(screen.getByLabelText("Grade level"), {
    target: { value: "Elementary" },
  });
  fireEvent.change(screen.getByLabelText("Questions or comments"), {
    target: { value: "Please arrange a visit." },
  });
  ...
  expect(within(status).getByRole("link")).toHaveAttribute(
    ...
    expect.stringContaining(
      "mailto:admissions.mdy@yismyanmar.com?subject=Campus%20tour%20request",
    ),
  );
  fireEvent.change(screen.getByLabelText("Phone"), {
    target: { value: "+959987654321" },
  });
  ...
  document
    ...
    .forEach((img) =>
      expect(img.getAttribute("src")).toMatch(
        /^\/(mandalay|partners)\/.+\.(jpg|png|webp)$/,
      ),
    );
});
