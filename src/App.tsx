...
export default function App() {
  if (
    /^\/mandalay\/?$/.test(window.location.pathname) &&
  ...
  if (/^\/welcome-messages\/?$/.test(window.location.pathname))
    return <WelcomeMessagesPage />;
  if (/^\/activities\/?$/.test(window.location.pathname))
  ...
  if (/^\/book-tour\/?$/.test(window.location.pathname))
    return <BookTourPage />;
  if (/^\/admissions\/?$/.test(window.location.pathname))
    return <AdmissionsPage />;
  return <HomePage />;
}
